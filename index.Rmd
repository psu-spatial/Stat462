...
Welcome to STAT-462 labs.  The aim of this week is to:
 ...
 e. Do some exploratory analysis on a data-set of credit scores
...
## Lab challenge 1 - R projects {#project}
...
It is ***really*** important that you stay organised in this course, for example making sure you know where you put your data and your labs. 
...
Now you have opened your project, take a screenshot of your R-studio page. It should look like Figure \@ref(fig:projectout), e.g. with at least R version 4.0.3, with the Lab 1 project and the project stored in your STAT-462 folder.
...
One example of a function is `file.choose()` (not how I put the parenthese in this lab book so you can see it is a command).  This command will let you interactively select a file and print the address out onto the screen. 
...
A close analogy is your phone.  There are millions of apps available from banking, to social media to camera filters.  You don't have every app in the world installed on your phone - and you don't have every the ones you *do* download running at the same time.  
...
This is the same for R:
 ...
 - To actually run the commands in the package you need to load them - just in the way you tap an app to start it. This can be done using the `library()` command.
...
If it gives you warnings about rtools, ignore them.

```{r, eval=FALSE, echo=TRUE}

# COPY/PASTE THESE INTO YOUR CONSOLE AND LET THEM RUN. 
#(These are hopefully all the packages for the course, so it's a one off)
...
## R-Markdown and exploratory analysis
...
This command compiles the summary statistics for the table you just saw. (you applied the *skim* command to the mtcars table).  
...
We want to see if our data is normally distributed.  We can do this using the histogram and normal-qq plot.   The command to do a histogram is `hist()` and `qqnorm()`

Create a new code chunk. Type or copy the following code into R.
...
**Question E:**   *In the text, in your own words, describe what a normal QQ-plot is and how to calculate one (if you're not sure, do a google search - there are good videos!).  In our case, why is it difficult for us to tell if the data comes from a population with a normal distribution?* =
...
For example, Cat lifespans have a normal distributon with a mean of 15.4 and a standard deviation of 2.3. What's the probability of a cat lifespan less than 14 years?
...
It's the same as we found by hand, about 0.27%.
...
**Question F:**  *Use R to find the probability below:*

 - *If X ∼ N(5,2), then find P(X ≤ 4)* 

**Question G:** *Use R to find the value of the normal quantiles:*

 - *If X ∼ N(5,2), then find the value a such that P(X < a) = 0.025* 

**Question H.** *Records maintained by the your admin office indicate that amount of time elapsed between the submission of travel receipts and the final reimbursement of funds has approximately a normal distribution with a mean of 39 days and a standard deviation of 6 days.*  
*If you submitted your travel claim 55 days ago, what is the probability that it should have been returned by now?  What might you conclude? You can do this either by hand or in R.* 
...
**Question J.** *Finally, let's include that project screenshot to prove to me you are using projects*  
...
**OPTIONAL Question K [BONUS 2%]**:  Finally, if you would like your histograms and plots to look more professional, take a look at ggplot and see if you can make a histogram the mpg data using this approach instead. This is a good tutorial: https://www.r-graph-gallery.com/220-basic-ggplot2-histogram.html]
...
Your code and document is neat and easy to read. LOOK AT YOUR HTML FILE IN YOUR WEB-BROWSWER BEFORE YOU SUBMIT.  There is also a spell check next to the save button.  
